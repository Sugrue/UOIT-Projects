			...
			int (*okfn)(struct sk_buff*))
{
	...
	if (iph->protocol == IPPROTO_ICMP)						//check the packets layer 4 protocol, if the packet is an ICMP packet, drop it and issue a debugging message
	{
		//printk("blocked an ICMP  packet\n");
		return NF_DROP;
	}
