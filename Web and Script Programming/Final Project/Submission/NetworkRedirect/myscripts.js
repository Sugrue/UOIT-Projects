...
function pageLoad() {
    ...
    $.get("http://api.bootswatch.com/3/", function(data) {
        ...
        select.change(function() {
            ...
            $("#css").attr("href", theme.css);
            //setCookie("theme",theme.name,30);	


            //requestSp(new Array(ID + ".theme.set." + (theme.name)),'setTheme');
...
alert(themeName[0]);
setCookie("theme",themeName[0],30);	
...
function loginVerify(login) {
    ...
    if (login[0] == '<i class="fa fa-thumbs-up"></i> Login Successful') {
    ...
    if (login[0] == '<i class="fa fa-exclamation-triangle"> </i> Incorrect Username and/or Password') {
...
function redirectFunction() {
    ...
    requestSp(new Array(ID + ".PC." + currentPC + ".network.redirect." + location),redirectFunctionVerify());
...
function redirectFunctionverify(){
